...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($client && !password_verify($currentPassword, $client['password'] ?? '')) {
        $errorMessage = 'Current password is incorrect.';
    } else {
        $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);
        if ($clientService->updatePassword($clientId, $hashedPassword)) {
...
<head>
    ...
    <style>
        *{ margin:0; padding:0; box-sizing:border-box; }
        ...
        .password-card{ background:#fff; max-width:700px; padding:35px; border-radius:20px; box-shadow:0 5px 20px rgba(0,0,0,0.08); }
        .password-card h2{ margin-bottom:25px; }
        ...
        .tips{ background:#fff3cd; color:#856404; padding:18px; border-radius:12px; margin-bottom:25px; line-height:1.7; }
...
<div class="main">
    ...
    <div class="password-card">
        <div class="tips">Use a strong password with a mix of uppercase letters, lowercase letters, numbers, and symbols for better security.</div>
